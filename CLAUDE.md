...
### Hero board — SAMPLE at `/board-preview.html` (unlisted, noindex)
An interactive 3D chessboard hero the user is evaluating as the possible final homepage hero. Files: `board-preview.html` (the homepage minus the product carousel, with the board hero on top), `js/hero-board.js` (three.js, vendored in `js/vendor`), `css/hero-board.css`. Nothing links to it; the old `hero-board-preview` git branch is stale (Vercel branch previews sit behind login, so the sample lives on `main`). **Look = the user's reference photo: dark, clean, glossy, realistic** — piano-black and silvery-ivory lacquer pieces, a glossy checker board that runs on past 8×8 and fades into black (scene fog), see-through squares with upside-down "twin" pieces as reflections (`addMirror`), dark studio environment with strip lights, low camera (29°), a toppled black king as scenery, pieces drawn oversized (`SCALE`) and spaced so each reads clearly. Six white pieces are the navigation: hover = lift + brass ring + ivory label + matching key highlights; click = the piece makes its own chess move (`MOVES`), camera dives, page dissolves, then navigates (King→king, Queen→queen, Pawn→pawn, Bishop→bishop, Rook→rook, Knight→book-a-fitting). Key row under the board = real links (keyboard/touch/no-WebGL/reduced-motion fallback). Test hook `window.__hb` (advance/click/hover/noNav; chained tweens need `await Promise.resolve()` between `advance` calls). Pending user decisions: whether it replaces the live hero, whether the carousel returns below it, how game-like to make it. To remove the sample delete `board-preview.html`, `css/hero-board.css`, `js/hero-board.js`.
