...
### Hero board (branch `hero-board-preview` — SAMPLE, not on `main`)
`js/hero-board.js` + the `.hero-board` block in `index.html`: an interactive 3D chessboard hero (three.js, procedural Staunton pieces, brass collars on the clickable white pieces). Hover = lift + brass ring + label; click = the piece makes its own chess move (king steps, rook slides, bishops/queen on diagonals, knight in an L, pawn two squares), camera dives into the board, page dissolves, then navigates (King→king.html, Queen→queen.html, Pawn→pawn.html, Bishop→bishop.html, Rook→rook.html, Knight→book-a-fitting.html). A key row under the board mirrors the pieces as real links (keyboard/touch/no-WebGL/reduced-motion fallback). Test hook `window.__hb` (advance/click/hover/noNav). The product showcase carousel sits below it on this branch.
