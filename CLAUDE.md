...
- **CSS cache-busting:** `css/style.css?v=N` query param on every page — bump `N` on every CSS change and update it across *all* HTML files (`sed` one-liner) **and** `CSS_VERSION` in `tools/gen_pawn_pages.py`, or browsers will serve stale styles. Currently **v=31**. `css/style.css` is one long file made of appended blocks — when replacing a block, replace only that block's span (an earlier slicing edit once deleted the blocks after it).
...
### Bag & checkout preview (`js/cart.js`, `js/checkout.js`, `checkout.html`)
Working client-side bag + a pre-checkout, no backend. **Nothing is sent or charged and no card details are collected** — payment is an explicit preview.
- `js/cart.js` loads on every page (after `main.js`): saves the bag in `localStorage` (`checkmatela.bag.v1`), wires the header bag button (`aria-label="Bag"`) + count badge, the slide-out bag drawer (qty steppers, remove, free-shipping meter at `FREE_SHIP`=$500, subtotal, Checkout), and adds "Add to bag" (+ size select on Rook) to the Bishop/Rook product cards. API: `window.CheckmatelaBag` (`add/remove/setQty/clear/items/count/subtotal/open/close/onChange`).
- Quick View (`js/shop.js`) shows the product's option chips from `data-options` on the `[data-shop]` wrapper (King: jacket size 36–48 + length; Pawn: size 2T–16) and an **Add to bag** button (required options are validated). Options come from `tools/gen_king_page.py` (`OPTIONS`) and `tools/gen_pawn_pages.py` (`KID_OPTIONS`).
- `checkout.html` is generated by `build_checkout()` in `tools/gen_info_pages.py`; logic in `js/checkout.js`: contact + delivery form with inline validation (email, ZIP, required fields), shipping methods (standard free ≥$500 else $18, express $35, studio pickup free), a demo promo code (`CHECKMATE10` = 10%), est. tax 9.5%, order summary with qty controls, and a **confirmation preview** (no order is placed). Rates/codes are placeholders in `RATES`.
- Note: a separate assistant built a different "sample bag" on the unmerged branch `codex/checkmatela-storefront` (its `js/main.js` / `js/catalog.js`); this implementation lives on `main`. If that branch is ever merged, reconcile the two bags (one `localStorage` key each).
